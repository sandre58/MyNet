<div align="center">

# MyNet.Geography.Localization

<img src="../../assets/MyNetGeography.png" alt="MyNet.Geography.Localization" width="96" height="96" />

*Localized country display names and MyNet.Humanizer formatters for geography types in MyNet.Geography.*

</div>

<div align="center">

[![MIT License](https://img.shields.io/github/license/sandre58/MyNet)](https://github.com/sandre58/MyNet/blob/main/LICENSE)
[![NuGet](https://img.shields.io/nuget/v/MyNet.Geography.Localization)](https://www.nuget.org/packages/MyNet.Geography.Localization)
[![.NET](https://img.shields.io/badge/.NET-10.0-512BD4)](https://dotnet.microsoft.com/download/dotnet/10.0)

</div>

---

## Features

| Feature | Description |
| :------ | :---------- |
| **Display names** | Localized country and region labels |
| **Resources** | Satellite .resx files for multiple cultures |
| **Humanizer** | Formatters for geography types |
| **Extension** | Culture-aware labels for MyNet.Geography |

---

## Installation

```bash
dotnet add package MyNet.Geography.Localization
```

## Quick start

Add the package when you need **translated country names** in UI or reports. Register Humanizer/geography formatters per [Geography guide](https://github.com/sandre58/MyNet/blob/main/docs/guides/geography.md).




---
## Related packages

- [MyNet.Geography](https://www.nuget.org/packages/MyNet.Geography)
- [MyNet.Humanizer](https://www.nuget.org/packages/MyNet.Humanizer)




---
## Documentation

- [Geography guide](https://github.com/sandre58/MyNet/blob/main/docs/guides/geography.md)
- [Humanizer guide](https://github.com/sandre58/MyNet/blob/main/docs/guides/humanizer.md)
---

<div align="center">

<sub>

Copyright © 2016-2026 - Stéphane ANDRE. All Rights Reserved.

<br/>

Released under the [MIT License](https://github.com/sandre58/MyNet/blob/main/LICENSE).

</sub>

</div>
