    ...
    where T : notnull
{
    ...
    public ExtendedCollection(SourceEngine<T> source, IScheduler? scheduler = null)
    {
        ...
        [
            _sourceObservable.ObserveOnOptional(scheduler).Bind(out _sortedSource).Subscribe(_ => ProcessPropertyChanged(nameof(SourceCount))),
    ...
    /// <summary>
    /// Gets the currently active grouping properties applied to the collection.
    /// </summary>
    public IGroupingProperty<T>[] CurrentGrouping => _groupingEngine.Current;

    /// <summary>
    /// Returns an observable sequence that emits the current groups whenever the items or grouping configuration changes.
    /// Emits an empty list when no grouping is active.
    /// </summary>
    public IObservable<IReadOnlyList<CollectionGroup<T>>> ConnectGroups() => _groupsObservable;

    /// <summary>
    /// Sets the grouping properties for the collection. The collection will reactively recompute its groups.
    /// </summary>
    /// <param name="grouping">The grouping properties to apply.</param>
    public void SetGrouping(params IGroupingProperty<T>[] grouping) => _groupingEngine.Set(grouping);

    /// <summary>
    /// Clears all grouping properties, producing an empty groups list.
    /// </summary>
    public void ClearGrouping() => _groupingEngine.Clear();
