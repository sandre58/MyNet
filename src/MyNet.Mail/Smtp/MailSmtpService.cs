...
public sealed class MailSmtpService(SmtpClient smtpClient) : IMailService, IDisposable
{
    ...
    public static MailMessage CreateMailMessage(IEmail email)
    {
        ...
        {
            message.To.Add(new MailAddress(address.Address, address.Name));
        }

        foreach (var address in data.Cc)
        {
            message.CC.Add(new MailAddress(address.Address, address.Name));
        }

        foreach (var address in data.Bcc)
        {
            message.Bcc.Add(new MailAddress(address.Address, address.Name));
        }

        foreach (var address in data.ReplyTo)
        {
            message.ReplyToList.Add(new MailAddress(address.Address, address.Name));
        }
        ...
        foreach (var attachment in data.Attachments)
        {
            ...
            var a = new Attachment(attachment.Data, attachment.Filename, attachment.ContentType) { ContentId = attachment.ContentId };
