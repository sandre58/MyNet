...
public abstract class EditionViewModel : WorkspaceViewModel
{
    ...
    {
        CancelCommand = RelayCommandFactory.Default.Create(async () => await CancelAsync().ConfigureAwait(false));
        SaveCommand = RelayCommandFactory.Default.Create(() => Save(), CanSave);
        SaveAsyncCommand = RelayCommandFactory.Default.Create(async () => await SaveAsync().ConfigureAwait(false), CanSave);
        SaveAndCloseCommand = RelayCommandFactory.Default.Create(SaveAndClose, CanSave);
        SaveAndCloseAsyncCommand = RelayCommandFactory.Default.Create(async () => await SaveAndCloseAsync().ConfigureAwait(false), CanSave);
