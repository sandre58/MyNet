...
public class DialogService(
    ...
    IMessageBoxFactory messageBoxFactory) : IDialogService
{
    ...
    public bool HasOpenedContentDialogs => OpenedContentDialogs.Any();
        ...
        where T : IDialogViewModel
    {
        ...
        await _contentDialogService.ShowAsync(viewModel, closeAction).ConfigureAwait(false);
    }
    ...
    {
        var viewModel = ViewModelManager.Get<T>();
        await ShowAsync(viewModel, closeAction).ConfigureAwait(false);
    }

    /// <inheritdoc />
    public async Task ShowAsync(Type typeViewModel, Action<IDialogViewModel>? closeAction = null)
    {
        var viewModel = (IDialogViewModel)ViewModelManager.Get(typeViewModel);
        await ShowAsync(viewModel, closeAction).ConfigureAwait(false);
    }
    ...
    public async Task<bool?> ShowModalAsync<T>(T viewModel)
        where T : IDialogViewModel
    {
        Messenger.Default?.Send(new OpenDialogMessage(DialogType.ModalDialog, viewModel));

        return await _contentDialogService.ShowModalAsync(viewModel).ConfigureAwait(false);
    }

    /// <inheritdoc />
    public async Task<bool?> ShowModalAsync<T>()
        where T : class, IDialogViewModel
        => await ShowModalAsync(ViewModelManager.Get<T>()).ConfigureAwait(false);

    /// <inheritdoc />
    public async Task<bool?> ShowModalAsync(Type typeViewModel)
        => await ShowModalAsync((IDialogViewModel)ViewModelManager.Get(typeViewModel)).ConfigureAwait(false);
    ...
    public Task<MessageBoxResult> ShowSuccessAsync(string message, string? title = null, MessageBoxResultOption buttons = MessageBoxResultOption.Ok)
        => ShowMessageAsync(message, title, buttons, MessageSeverity.Success);
    ...
    public Task<MessageBoxResult> ShowInformationAsync(string message, string? title = null, MessageBoxResultOption buttons = MessageBoxResultOption.Ok)
        => ShowMessageAsync(message, title, buttons, MessageSeverity.Information);
    ...
    public Task<MessageBoxResult> ShowErrorAsync(string message, string? title = null, MessageBoxResultOption buttons = MessageBoxResultOption.Ok)
        => ShowMessageAsync(message, title, buttons, MessageSeverity.Error);
    ...
    public Task<MessageBoxResult> ShowWarningAsync(string message, string? title = null, MessageBoxResultOption buttons = MessageBoxResultOption.Ok)
        => ShowMessageAsync(message, title, buttons, MessageSeverity.Warning);

    /// <inheritdoc />
    public Task<MessageBoxResult> ShowQuestionAsync(string message, string? title = null)
        => ShowMessageAsync(message, title, MessageBoxResultOption.YesNo, MessageSeverity.Question);

    /// <inheritdoc />
    public Task<MessageBoxResult> ShowQuestionWithCancelAsync(string message, string? title = null)
        => ShowMessageAsync(message, title, MessageBoxResultOption.YesNoCancel, MessageSeverity.Question);
